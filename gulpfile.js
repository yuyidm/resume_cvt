...
const sourcemaps = require('gulp-sourcemaps'); // 映射
...
// 获取配置文件
const { createTemplate } = require('./config.js');
...
function sassDev() {
    return gulp
        ...
        .pipe(sourcemaps.write('../sass_map'))
        .pipe(gulp.dest('./dist'));
}
 ...
 */
function imgDev() {
    return gulp.src(['template/**/*.[png|svg|jpg|jpeg]']).pipe(gulp.dest('./dist'));
}
